...
export function TutorSchedulePage({ userRole, onNavigate, onGoBack }: TutorSchedulePageProps) {
  ...
  const tutorName = 'PGS.TS Nguyễn Thành Công'; // Matching the Sidebar prop
  ...
  const handleSlotClick = (day: string, date: string, hour: string, status: any) => {

    if (status === 'available') {
      setSlotToCancel({ day, hour });
      handleConfirmCancel();
    } else if (status === null) {
      setSlotToDefine({ day, date, hour, status });
