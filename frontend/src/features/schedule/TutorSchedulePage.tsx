...
import { cancelAppointment, getScheduleForTutor } from './api/calendarApi';
                              ...
                              return (
                                <td key={`${dayData.date}-${hour}`} className="p-0 border-b border-r last:border-r-0">
                                  <button onClick={() => handleSlotClick(dayData.day, dayData.date, hour, slot ?? null)}
                                    className={`w-full h-full p-2 text-left transition-colors min-h-[60px] flex items-center justify-center ${slot ? 'bg-red-100 border-red-200 hover:bg-red-200 cursor-pointer' : ''
                                      }`}
                                  >
                                    {slot && (
