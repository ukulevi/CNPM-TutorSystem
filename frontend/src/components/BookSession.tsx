...
import { Button } from './ui/button'; // Giả sử file này sẽ được di chuyển sau
import { Card, CardContent } from './ui/card'; // Giả sử file này sẽ được di chuyển sau
import { Avatar, AvatarFallback } from './ui/avatar'; // Giả sử file này sẽ được di chuyển sau
import { Badge } from './ui/badge'; // Giả sử file này sẽ được di chuyển sau
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  DialogHeader,
  DialogTitle,
} from './ui/dialog'; // Giả sử file này sẽ được di chuyển sau
...
export function BookSession({ tutor, currentUserId, currentUserName, onNavigate }: BookSessionProps) {
  const [schedule, setSchedule] = useState<Session[]>([]); // Giả sử lịch hẹn có kiểu Session
  ...
  const handleConfirmBooking = async () => {
    ...
    await createSession(sessionData as Omit<Session, 'id'>);
        ...
        <DialogContent>
          ...
          <div className="py-4">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              {(selectedSlot as any)?.type === 'offline' ? (
                <>
                  <MapPin className="w-4 h-4" />
                  <span>Buổi học Trực tiếp</span>
                </>
              ) : (
                <>
